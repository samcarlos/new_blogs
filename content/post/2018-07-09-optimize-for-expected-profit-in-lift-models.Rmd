...
author: Sam Weiss
date: '2018-07-09'
slug: optimize-for-expected-profit-in-lift-models
categories: []
tags:
  - R
---
...
$$Expected Profit = \sum_{k=1}^{K}{I(optim_{i,k})*y_{i}(x,t=optim_{i,k})}$$
...
$AverageProfit_{i} =  \frac{\sum_{i=k}^{K}{I(optim_{i} = t_{i})*y_{i}}}{\sum_{i=k}^{K}{I(optim_{i} = t_{i})}}$
...
We cannot optimize this directly b/c the indicator function is non-differentiable. Instead replacing I() function with a probability and use the custom loss function below. - Thanks to [Matt Becker](https://github.com/beckermr) for this insight. 

$$profit_{i} =  \sum_{k=1}^{K}{\hat{p_{i,k}(x)}*I(t_i=k)*y_{i}}$$
...
[Comments](http://scweiss.blogspot.com/2018/07/comments-for-optimize-for-expected.html)